...
class DQN_Dynamics_Normal(DQN_Agent):
    def __init__(self, env):
        ...
        self.state_total_X = np.zeros(self.get_observation_space().shape[0])
        self.state_total_X_2 = np.zeros(self.get_observation_space().shape[0])
        self.state_N = 0
    ...
    def update_model(self, state, action, reward, new_state, done):
        ...
        self.state_total_X += state[0]
        self.state_total_X_2 += np.power(state[0], 2)
        self.state_N += 1
        self.fit_q_network()
    ...
    def explore(self,state):
        ...
        mean = self.state_total_X/self.state_N
        mean_X2 = self.state_total_X_2/self.state_N
        var = mean_X2 - np.power(mean,2)
        #print(mean)
        least_p = 99999.
        best_a = None
        ...
        for action in range(self.get_action_space().n):
            ...
            p = 1.
            for i, s in enumerate(next_state[0]):
                p *= stats.norm(mean[i], var[i]).pdf(s)
